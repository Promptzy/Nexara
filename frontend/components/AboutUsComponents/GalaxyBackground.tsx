...
export const GalaxyBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const drawNebulae = () => {
      nebulaeRef.current.forEach(nebula => {
        const pulse = Math.sin(
          timeRef.current * nebula.pulseSpeed + nebula.pulsePhase
        )
        ...
        gradient.addColorStop(
          0,
          nebula.color.replace(/[\d.]+\)$/g, `${currentOpacity})`)
        )
        gradient.addColorStop(
          0.4,
          nebula.color.replace(/[\d.]+\)$/g, `${currentOpacity * 0.6})`)
        )
    ...
    const drawStarField = () => {
      ...
      for (let i = 0; i < 200; i++) {
        const x =
          (Math.random() * canvas.width + timeRef.current * 10) % canvas.width
        const y =
          (Math.random() * canvas.height + timeRef.current * 5) % canvas.height
    ...
    const drawShootingStars = () => {
      shootingStarsRef.current.forEach(star => {
        ...
        glowGradient.addColorStop(
          0,
          `rgba(255, 255, 255, ${star.opacity * 0.3})`
        )
